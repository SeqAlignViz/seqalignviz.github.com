// Adapted from MEME example (http://meme.sdsc.edu/meme/doc/examples/meme_example_output_files/meme.html).
...
function LogoMetrics(ctx, canvas_width, canvas_height, alphabet, positions, use_preferred_and_scale, include_scale, include_numbers, extraPadLeft, extraPadRight) {
  if (use_preferred_and_scale === undefined) use_preferred_and_scale = false;
  if (extraPadLeft === undefined) { extraPadLeft = 0; }
  if (extraPadRight === undefined) { extraPadRight = 0; }
  //variable prototypes
  ...
  this.scale_y = 1;
  this.pad_top = 1;
  this.pad_left = 1 + extraPadLeft;
  this.pad_right = 1 + extraPadRight;
  this.pad_bottom = 1;
  this.y_label = "bits"
  ...
  this.stack_font = "bold 20px arial,sans-serif";
  this.stack_height = 0;
  this.stack_width = 0;
  this.stacks_pad_right = 0;
  this.x_num_above = 2;
  ...
  var label_consumed_height = 0;
  if (include_numbers) {
    label_consumed_height += this.x_num_height + this.x_num_above;
    if (positions >= 10) {
      label_consumed_height += this.x_num_height;
    }
    if (positions >= 100) {
      label_consumed_height += this.x_num_height;
    }
  }
  var scale_consumed_height = 0;
  if (include_scale) {
    scale_consumed_height += this.y_num_height;
  }
  var extra_height = 0;
  extra_height = this.pad_bottom + Math.max(scale_consumed_height/2, label_consumed_height) + this.pad_top + scale_consumed_height;
  ...
  var extra_width = this.pad_left + this.stacks_pad_right + this.pad_right;
  if (include_scale) {
    extra_width += this.y_label_height + this.y_label_spacer + this.y_num_width + this.y_tic_width;
  }
  if (use_preferred_and_scale) {
...
function draw_logo(pspm, alphabet, image_el, include_scale, include_numbers, extraPadLeft, extraPadRight) {
  if (extraPadLeft === undefined) { extraPadLeft = 0; }
  if (extraPadRight === undefined) { extraPadRight = 0; }
  var image = image_el;
  if (!image) {
    alert("Can't find specified image.");
    return;
  }
  ...
  /* Discovered on May 4, 2011, that image.width, image.height sometimes don't have correct values in firefox
  canvas.width = image.width;
  canvas.height = image.height;
  */
  canvas.width = parseInt(YAHOO.util.Dom.getStyle(image_el, "width"));
  canvas.height = parseInt(YAHOO.util.Dom.getStyle(image_el, "height"));
  //canvas.width = image.width;
  //canvas.height = image.height;
  canvas.id = image.id;
  canvas.alt = image.alt;
  canvas.style.display = image.style.display;
  var metrics = new LogoMetrics(ctx, canvas.width, canvas.height, alphabet, pspm.get_motif_length(), false,
    include_scale, include_numbers, extraPadLeft, extraPadRight);
  ctx.save();
  ...
  ctx.translate(metrics.stack_width/2, 0);
  if (include_scale) {
  ...
  oImg.id = canvas.id;
  //replace the image with the new imge
  image.parentNode.replaceChild(oImg, image);
}
